...
一台机器:   聚合器（灯 query 的主服务）
             抓 statusd + Upptime + web → 合并 → 缓存 → /status（端口 8080）
                ↓ HTTP（公网，供家里+公司两块灯板）
...
- `name` = **可读名**：upptime 组名 / `web-alive` / 机器信号带 host 前缀（`laborari:docker`，防跨机重名）。不搞代号层（域名/机器名本就公开）
...
- `listen`、`sources[]`（upptime / statusd / web）、每源 `refresh` 秒
